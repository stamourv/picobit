  ...
  (lambda ()
    (list (make-var '#%number? #t '() '() '() #f (make-primitive 1 #f #f))
	  ...
	  (make-var '#%set-trd! #t '() '() '() #f (make-primitive 2 #f #f)) ;; ADDED
          ...
          (make-var '#%motor #t '() '() '() #f (make-primitive 3 #f #t))
          (make-var '#%led #t '() '() '() #f (make-primitive 1 #f #t))
...
;    (pp (list var: var local: (stack-slots (env-local (context-env ctx))) (env-closed (context-env ctx))))
    (let ((i (find-local-var var (context-env ctx))))
      (if (>= i 0)
          ...
          (gen-push-stack (+ (- -1 i) (length (stack-slots (env-local (context-env ctx))))) ctx)))))
...
(define gen-closure
  (lambda (label-entry ctx)
    (gen-instruction (list 'closure label-entry) 2 1 ctx)))
